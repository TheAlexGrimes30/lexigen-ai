...
class ContractRiskAnalysisPromptBuilder(BasePromptBuilder):
    ...
    def build(
        ...
        Твоя задача — провести анализ текста договора
        на юридические риски, спорные условия, пробелы
        и дать рекомендации по исправлению.

        Используй ТОЛЬКО:
        1. Текст договора из блока "ДОГОВОР"
        2. Нормы права из блока "КОНТЕКСТ"

        НЕ используй внешние знания.
        НЕ выдумывай статьи закона.
        НЕ ссылайся на нормы, которых нет в контексте.
        НЕ объясняй ход мыслей.
        НЕ используй слова: "сначала", "проверяю", "анализирую".
        ...
        2. Основные риски договора
        - Риск:
        - Почему это риск:
        - Возможные последствия:
        - Связанные нормы из контекста:

        3. Спорные или слабые условия
        - Условие:
        - Проблема:
        - Рекомендация:
        ...
        - Что отсутствует:
        - Почему важно добавить:
        - Как лучше сформулировать:

        5. Рекомендации по доработке договора
        - Рекомендация:
        - Ожидаемый эффект:
        ...
        ЕСЛИ В КОНТЕКСТЕ НЕТ ПОДХОДЯЩИХ НОРМ:
        напиши:
        "В базе знаний недостаточно норм для полного правового анализа."
