...
class MessagesService:
    ...
    async def create_message_turn_response(
        ...
        if file is not None:
            user_message, assistant_message = await self.create_document_analysis_turn(db=db, chat_id=chat_id, user_id=chat.user_id, content=content, file=file)
        else:
            user_text = (content or "").strip()
            if not user_text:
                raise HTTPException(status_code=400, detail="Сообщение не может быть пустым")
            user_message, assistant_message = await self.create_dialog_turn(db=db, chat_id=chat_id, user_id=chat.user_id, user_text=user_text)
    ...
    async def create_dialog_turn(
        ...
        user_message = Message(chat_id=chat_id, user_id=user_id, role=MessageRole.user, content=user_text)
        db.add(user_message)
        await db.flush()
        ...
        assistant_message = Message(chat_id=chat_id, user_id=user_id, role=assistant_role, content=assistant_text)
    ...
    async def create_document_analysis_turn(
        ...
        chat_document = ChatDocument(chat_id=chat_id, uploaded_by=user_id, filename=original_filename, original_filename=original_filename, mime_type=file.content_type, extracted_text=extracted_text)
        ...
        if not user_content:
            ...
            user_content = f"{user_content} Файл: {original_filename}"

        user_message = Message(chat_id=chat_id, user_id=user_id, role=MessageRole.user, content=user_content, chat_document_id=chat_document.id)
        db.add(user_message)
        await db.flush()
        ...
        analysis_result = AnalysisResult(chat_id=chat_id, document_id=chat_document.id, generated_by_user_id=user_id, summary=analysis_text, risks_found=None, recommendations=None)
        db.add(analysis_result)
        await db.flush()

        assistant_message = Message(chat_id=chat_id, user_id=user_id, role=assistant_role, content=analysis_text, chat_document_id=chat_document.id, analysis_result_id=analysis_result.id)
        db.add(assistant_message)
        await db.commit()
        await db.refresh(user_message)
        await db.refresh(assistant_message)
    ...
    async def create_system_error_response(
        ...
        if not chat:
            raise HTTPException(status_code=404, detail="Чат не найден")
        system_message = Message(chat_id=chat_id, user_id=chat.user_id, role=MessageRole.system, content=error_text)
    ...
    def _validate_document(self, file: UploadFile) -> None:
        ...
        if suffix not in {".docx", ".pdf"}:
            raise HTTPException(status_code=400, detail="Можно загрузить только DOCX или PDF")
